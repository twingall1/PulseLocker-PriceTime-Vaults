...
contract GenericPriceTimeVault {
    ...
    function withdraw() external {
        ...
        if (isNative) {
            ...
            if (fee > 0) {
                (bool s1, ) = payable(FEE_RECIPIENT).call{value: fee}("");
                require(s1, "fee fail");
            }
            (bool s2, ) = payable(owner).call{value: payout}("");
            require(s2, "send fail");
        } else {
            ...
            if (fee > 0) {
                require(
                    IERC20Generic(lockToken).transfer(FEE_RECIPIENT, fee),
                    "fee fail"
                );
            }
            require(
                IERC20Generic(lockToken).transfer(owner, payout),
                "send fail"
            );
        }
        ...
        function rescue(address token) external {
            ...
            require(
                IERC20Generic(token).transfer(owner, bal),
                "rescue fail"
            );
        }
